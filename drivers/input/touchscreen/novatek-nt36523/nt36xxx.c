...
static unsigned long last_cksum_err = 0;
...
static inline int32_t i2c_read_write(struct i2c_client *client, uint8_t *buf, size_t len, NVT_SPI_RW rw)
{
	struct i2c_msg m;
	int ret;

	/* Novatek SDK I2C protocol (matches stock nt36xxx I2C builds):
	 *  - read: write 1 index byte, then read len-1 data bytes
	 *  - write: index byte + data in ONE transaction (no STOP between) */
	m.addr = client->addr;
	m.flags = 0;
	...
	switch (rw) {
		case NVTREAD:
		{
			struct i2c_msg msgs[2] = {
				...
				{ .addr = client->addr, .flags = I2C_M_RD, .len = len - 1, .buf = buf + 1 },
			};
...
int32_t CTP_SPI_READ(struct i2c_client *client, uint8_t *buf, uint16_t len)
{
	int32_t ret = -1;
	int32_t retries = 0;
	...
	while (retries < 5) {
		ret = i2c_read_write(client, buf, len, NVTREAD);
		if (ret == 0) break;
		retries++;
	}
	...
	if (unlikely(retries == 5)) {
		...
		/* data returned in place: buf[1]..buf[len-1] (I2C has no dummy byte) */
	}
...
int32_t CTP_SPI_WRITE(struct i2c_client *client, uint8_t *buf, uint16_t len)
{
	int32_t ret = -1;
	int32_t retries = 0;
	...
	while (retries < 5) {
		ret = i2c_read_write(client, buf, len, NVTWRITE);
		if (ret == 0)	break;
		retries++;
	}
...
static int32_t nvt_parse_dt(struct device *dev)
{
	...
	if (IS_ERR(ts->rst_gpio)) {
		NVT_ERR("failed to request reset GPIO\n");
		return PTR_ERR(ts->rst_gpio);
	}
	if (ts->rst_gpio)
		NVT_LOG("reset-gpios ok\n");
#endif
	ts->irq_gpio = ts->client->irq; /* from "interrupts" property */
	ts->irq_flags = 0;
	NVT_LOG("irq=%d\n", ts->irq_gpio);

	ts->pen_support = false; /* pen input device not ported */

	ret = of_property_read_u32(np, "novatek,swrst-n8-addr", &SWRST_N8_ADDR);
	if (ret) {
		/* NT36523 default (matches Samsung downstream dtsi) */
		SWRST_N8_ADDR = 0x3F0FE;
	} else {
...
{
#if NVT_TOUCH_SUPPORT_HW_RST
	/* reset GPIO is owned via devm_gpiod_get_optional() in nvt_parse_dt() */
	if (ts->rst_gpio)
...
static int32_t nvt_ts_point_data_checksum(uint8_t *buf, uint8_t length)
{
   ...
   if (checksum != buf[length]) {
       if (time_after(jiffies, last_cksum_err + msecs_to_jiffies(1000))) {
           last_cksum_err = jiffies;
		...
		for (list = 0; list < (sizeof(trim_id_table) / sizeof(struct nvt_ts_trim_id_table)); list++) {
			...
			if (found_nvt_chip) {
				...
				if (list == 0 && retry >= 2) {
					...
					ts->carrier_system = 0;
					ts->hw_crc = 0;
					NVT_LOG("proceeding with NT36525 default (retry %d)\n", retry);
					ret = 0;
					goto out;
				}
				ts->mmap = NULL;
...
static int32_t nvt_ts_probe(struct i2c_client *client)
{
	...
	if (ts == NULL) {
		NVT_ERR("failed to allocate memory for nvt ts data\n");
		return -ENOMEM;
	}
	...
	/* Boot handshake: take the chip out of bootloader, wait for app fw to
	 * boot, then read fw info. nvt_read_pid() leaves the xdata bank on
	 * EVENT_BUF_ADDR which the ISR relies on (it reads index 0x00 with no
	 * nvt_set_page of its own). */
	nvt_write_addr(ts->mmap->EVENT_BUF_ADDR | EVENT_MAP_RESET_COMPLETE, 0x00);
	nvt_boot_ready();
	ret = nvt_check_fw_reset_state(RESET_STATE_INIT);
	if (ret) {
		NVT_ERR("fw boot handshake failed, chip stuck in bootloader\n");
		ret = -EINVAL;
		goto err_chipvertrim_failed;
	}
	ret = nvt_get_fw_info();
	if (ret)
		NVT_ERR("nvt_get_fw_info failed, using fw defaults\n");
	...
	ts->int_trigger_type = irq_get_trigger_type(client->irq);
...
MODULE_DESCRIPTION("Novatek NT36523 Touchscreen Driver (I2C)");
MODULE_LICENSE("GPL");

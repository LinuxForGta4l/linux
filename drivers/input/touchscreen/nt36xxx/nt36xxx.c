...
int32_t CTP_SPI_READ(struct spi_device *client, uint8_t *buf, uint16_t len)
{
	...
	if (unlikely(retries == 5)) {
		...
		/* SPI: burst starts 1 dummy byte after the address byte */
   ...
   if (POINT_DATA_LEN >= POINT_DATA_CHECKSUM_LEN) {
	...
	for (i = 0; i < ts->max_touch_num; i++) {
		...
		if (((point_data[position] & 0x07) == 0x01) || ((point_data[position] & 0x07) == 0x02)) {	//finger down (enter & moving)
			...
			input_report_abs(ts->input_dev, ABS_MT_POSITION_X, ts->abs_x_max - input_x);
			input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, ts->abs_y_max - input_y);
...
static int8_t nvt_ts_check_chip_ver_trim(uint32_t chip_ver_trim_addr)
{
	...
	ts->carrier_system = 0;
	ts->hw_crc = 0;
	NVT_LOG("proceeding with NT36525 default\n");
	ret = 0;
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	/* SPI with 0  dummy byte: Novatek needs a zero byte after the
	 * address word on reads; spi_read_write() adds DUMMY_BYTES itself. */
	...
	if (ret) {
		NVT_ERR("fw boot handshake failed (chip in bootloader), fw update queued\n");
	}
	ret = nvt_get_fw_info();
	if (ret)

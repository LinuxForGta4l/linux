...
request_fail:
		...
		if(unlikely(retry > 2)) {
			/*
			 * Rootfs (/lib/firmware) may not be mounted yet when the
			 * boot update fires ~14 s after probe (the queue delay is
			 * only a display-reset wait, no fs dependency). -ENOENT
			 * means the file is not there yet: back off and retry
			 * instead of failing permanently while udev mounts rootfs.
			 */
			if (ret == -ENOENT && retry < 33) {
				msleep(1000);
				continue;
			}
...
int32_t nvt_update_firmware(char *firmware_name)
{
	...
	NVT_LOG("Update firmware success! <%lld us>\n",
			(long long)ktime_us_delta(end, start));
